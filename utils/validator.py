...
def validate_mandatory_params(config):
    ...
    for get_param_value, param_name in parameter_list:
        if get_param_value(config) == "":
            logger.error(
                f"validation failed: '{param_name}' must not be empty")
            is_valid = False
...
def validate_digit_params(config):
    ...
    if has_dedicated_proc(config) == "true":
        cpu_params = [
            (get_desired_proc,
             f"{flavor_name}.cpu.dedicated.desired-proc-unit"),
            (get_max_proc, f"{flavor_name}.cpu.dedicated.max-proc-unit"),
            (get_min_proc, f"{flavor_name}.cpu.dedicated.min-proc-unit")
        ]
    else:
        cpu_params = [
            (get_shared_desired_proc,
             f"{flavor_name}.cpu.shared.desired-proc-unit"),
            (get_shared_max_proc, f"{flavor_name}.cpu.shared.max-proc-unit"),
            (get_shared_min_proc, f"{flavor_name}.cpu.shared.min-proc-unit"),
            (get_shared_desired_virt_proc,
             f"{flavor_name}.cpu.shared.desired-virt-proc"),
            (get_shared_max_virt_proc,
             f"{flavor_name}.cpu.shared.max-virt-proc"),
            (get_shared_min_virt_proc,
             f"{flavor_name}.cpu.shared.min-virt-proc")
        ]
    ...
    for get_digit, param_name in parameter_list:
        if not get_digit(config).isdigit():
            logger.error(
                f"validation failed: '{param_name}' must be an integer")
            is_valid = False
    ...
    for get_memory_param, param_name in memory_params:
        ...
        if value.isdigit() and int(value) % 2 != 0:
            logger.error(
                f"validation failed: '{param_name}' must be an even integer")
            is_valid = False
...
def validate_prefix_length(config):
    ...
    if value.isdigit() and int(value) <= 32:
        return True
    logger.error(
        "validation failed: 'partition.network.ip.prefix-length' value must be less then or equal to 32.")
...
def validate_partition_name(config):
    ...
    if not bool(result):
        logger.error(
            f"validation failed: 'partition.name' value '{get_partition_name(config)}' have invalid format.")
    return bool(result)
...
def validate_partition_flavor(config):
    ...
    if value in possible_values:
        return True
    logger.error(
        f"validation failed: 'partition.flavor' must filled with {possible_values}.")
    return False
...
def validate_desired_memory(config):
    ...
    if value.isdigit() and int(value) >= 32:
        return True
    logger.error(
        f"validation failed: '{get_flavor_name(config)}.memory.desired-memory' must have greater then or equal to 32 GB of memory")
    return False
...
def validate_cpu_mode(config):
    ...
    if value in possible_values:
        return True
    logger.error(
        f"validation failed: '{get_flavor_name(config)}.cpu.mode' must filled with {possible_values}")
...
def validate_dedicated_desired_proc(config):
    ...
    if value.isdigit() and int(value) >= 1:
        return True
    logger.error(
        f"validation failed: '{get_flavor_name(config)}.cpu.dedicated.desired-proc-unit' must be greater then or equal to 1 proc-unit")
    return False
...
def validate_system_name(config):
    ...
    if not bool(result):
        logger.error(
            f"validation failed: 'system.name' value '{get_system_name(config)}' have invalid format.")
    return bool(result)
...
def validate_bootstrap_url(config):
    ...
    if not bool(result):
        logger.error(
            f"validation failed: 'ai.bootstrap-iso-url' value '{get_bootstrap_iso_download_url(config)}' must be a url")
    return bool(result)
...
def validate_json(value, field):
    if value != "":
        try:
            ...
            logger.error(
                f"validation failed: '{field}' must have valid json value. {e}")
            return False
    return True
...
def validate_ai_app_validator(config):
    ...
    if not bool(validate_url(url)):
        logger.error(
            f"validation failed: 'ai.validation.url' value '{url}' must be a url")
        result = False

    method = get_ai_app_method(config)
    if method != "GET" and method != "POST":
        logger.error(
            f"validation failed: 'ai.validation.method' value '{method}' must be either GET or POST")
        result = False
...
def validate_host_config(config, cookies, sys_uuid):
    return validate_mngt_console_version(config, cookies) and validate_lmb_size(config, cookies, sys_uuid)


def validate_mngt_console_version(config, cookies):
    uri = "/rest/api/uom/ManagementConsole"
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(
                f"failed to get management console version, error: {response.text}")
            raise Exception(
                f"failed to get management console version, error: {response.text}")

        soup = BeautifulSoup(response.text, "xml")
        version_tag = soup.find("VersionInfo")
        if version_tag == None:
            logger.error("failed to find version of management console.")
            raise Exception("failed to find version of management console.")
        power_version = version_tag.find("Version")
        service_pack_name = version_tag.find("ServicePackName")

        if int(power_version.text) < 10:
            logger.error("minimum power version '10' is expected")
            raise Exception("minimum power version '10' is expected")
        if int(service_pack_name.text) < 1050:
            logger.error("minimum firmware version '1050' is expected")
            raise Exception("minimum firmware version '1050' is expected")
    except Exception as e:
        raise e
...
def validate_lmb_size(config, cookies, sys_uuid):
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(
                f"failed to get management system details, error: {response.text}")
            raise Exception(
                f"failed to get management system details, error: {response.text}")

        soup = BeautifulSoup(response.text, "xml")
        lmb_size = soup.find("CurrentLogicalMemoryBlockSize")
        if lmb_size is None:
            logger.error("failed to find CurrentLogicalMemoryBlockSize value")
            raise Exception("failed to find CurrentLogicalMemoryBlockSize value.")

        if int(lmb_size.text) < 1024:
            logger.error(
                "insufficient memory for CurrentLogicalMemoryBlockSize, atleast '1024 MB' is expected")
            raise Exception(
                "insufficient memory for CurrentLogicalMemoryBlockSize, atleast '1024 MB' is expected.")
    except Exception as e:
        raise e
